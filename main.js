(() => {
  ...
  var textarea = document.getElementById("list_member");
  var submitButton = document.getElementById("button_submit");
  var inputField = document.getElementById("list_member");
  var valuesList = [];

  //=====< Danh sách phần thưởng >=====
  const listGift = [
    {
      text: "Nguyễn Hoàng Phúc Hải",
      percent: 80 / 100,
    },
    {
      text: "Siêu xe",
  ...
  const showGift = (gift) => {
    let timer = setTimeout(() => {
      ...
      showMsg.innerHTML = `Xin chúc mừng "${gift.text}"`;
